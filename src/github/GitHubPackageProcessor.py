...
class VersionAnalyzer:
    ...
    def process_package(self, row: List) -> Dict:
        ...
        try:
            ...
            latest_version_github = latest_release.get('tag_name') if latest_release else None
            ...
            if latest_release and arch_ext_pairs:
                all_urls = [url.replace('%2B', '+') for url in latest_release.get('asset_urls', [])]
                filtered_urls = []
                ...
                latest_github_urls = ''
            normalized_latest = latest_version_github.lower().lstrip('v') if latest_version_github else ''
            ...
            return {
                ...
                "ArchExtPairs": arch_ext_pairs,
                "HasOpenPullRequests": open_prs,
                "GitHubLatest": latest_version_github if latest_version_github else "Not found",
    ...
    def analyze_versions(self, input_path: Path, output_path: Path) -> None:
        try:
            ...
            df_filtered = df.filter(pl.col('LatestVersionURLsInWinGet').str.contains('github.com'))
            
            logging.info(f"Processing {len(df_filtered)} packages with GitHub repositories")
            
            # Process packages in parallel using ThreadPoolExecutor
            results = []
            with ThreadPoolExecutor(max_workers=10) as executor:
                # Submit all tasks and get futures
                future_to_row = {executor.submit(self.process_package, row): row 
