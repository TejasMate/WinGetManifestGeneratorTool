...
class PackageProcessor(YAMLProcessorBase):
    def __init__(self, config: ProcessingConfig):
        ...
        self.version_patterns: Dict[str, Set[str]] = {}  # Changed to Set to store multiple patterns
        ...
        self.architectures = [
            'x86-64', 'aarch64', 'x86_64',
            'arm64', 'arm', 'win64', 'amd64', 'x86', 'i386', 'ia32', '386', 'win32', '32bit',
            'win-arm64', 'win-x64', 'win-x86', 'win-ia32', 'windows-arm64', 'windows-x64', 'windows-x86', 'windows-ia32',
            'armv6', 'armv7', 'arm8', 'arm9', 'x64', 'x32', 'i686', '64bit',
            'x86_x64', 'x86only', 'shared-32', 'shared-64', 'installer32', 'installer64', '32', '64'
        ]
        self.extensions = ['msixbundle', 'appxbundle', 'msix', 'appx', 'zip', 'msi', 'exe']
    ...
    def calculate_max_dots(self, files: List[Path]) -> None:
        try:
            ...
            logging.error(f"Error calculating max dots: {e}")
            self.max_dots = 0
        ...
        for url in urls:
            ...
            for arch in self.architectures:
                ...
                if re.search(pattern, url_lower):
                    arch_match = arch
                    logging.info(f"Found architecture {arch_match} using pattern: {pattern}")
                    break
    ...
    def process_package(self, package_parts: List[str]) -> None:
        try:
            ...
            for version_dir in all_dirs:
                ...
                subdirs_with_yaml = any(any(f.is_file() and f.suffix == '.yaml' for f in d.iterdir()) 
                                      for d in version_path.iterdir() if d.is_dir())
            ...
            logging.error(f"Error processing package {package_name if 'package_name' in locals() else '.'.join(package_parts)}: {e}")

    def check_package_in_prs(self, package_name: str, pr_titles: List[str]) -> str:
        package_lower = package_name.lower()
        return 'present' if any(package_lower in title.lower() for title in pr_titles) else 'absent'

    def create_analysis_dataframe(self, pr_titles: Optional[List[str]] = None) -> pl.DataFrame:
        try:
            data = []
            for pkg, vers in self.package_versions.items():
                ...
                data.append({
                    ...
                    "VersionFormatPattern": ",".join(sorted(self.version_patterns.get(pkg, {"unknown"}))),
                    "CurrentLatestVersionInWinGet": self.latest_version_map.get(pkg, ""),
                    "InstallerURLsCount": self.package_downloads.get(pkg, 0),
                    "LatestVersionURLsInWinGet": ",".join(self.latest_urls.get(pkg, [])),
                    "ArchExtPairs": self.arch_ext_pairs.get(pkg, ""),
                    "HasOpenPullRequests": self.check_package_in_prs(pkg, pr_titles) if pr_titles else "unknown"
                })
            ...
            for item in data:
                ...
                if pkg in self.latest_urls and not self.arch_ext_pairs.get(pkg):
                    self.arch_ext_pairs[pkg] = self.extract_arch_ext_pairs(self.latest_urls[pkg])
    ...
    def process_files(self) -> None:
        try:
            ...
            for row in manifest_df.to_dicts():
                parts = [row[f'column_{i}'] for i in range(len(row)) if row[f'column_{i}']]
                if parts:
