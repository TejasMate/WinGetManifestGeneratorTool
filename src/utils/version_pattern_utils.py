    ...
    def get_version_pattern_name(version: str) -> str:
        ...
        for seg in segments:
            if seg in ['.', '-', '_', ' ', '(', ')']: # Handle separators
                pattern_parts.append('Dash' if seg == '-' else ('Dot' if seg == '.' else ('Underscore' if seg == '_' else ('Space' if seg == ' ' else ('OpenBrac' if seg == '(' else 'CloseBrac')))))
                ...
                for part in parts:
                    if part.isdigit():
                        ...
                        pattern_parts.append(f"{len(part)}Letter" if len(part) == 1 else VersionPatternDetector.standardize_dev_stage(part))

        # Handle numeric versions with separators and letter suffixes
        segments = re.split(r'([. _()-]|(?<=[0-9])(?=[a-zA-Z])|(?<=[a-zA-Z])(?=[0-9]))', version)
        if not segments:
            return "unknown"
        ...
        for i, seg in enumerate(segments):
            if seg in ['.', '-', '_', ' ', '(', ')']:
                if seg == ' ':
                    ...
                    pattern_parts.append('Dot' if seg == '.' else ('Dash' if seg == '-' else 'Underscore'))
            elif seg.isdigit():
    ...
    def determine_version_pattern(version: str, url_ext: Optional[str] = None) -> str:
        try:
            ...
            if pattern_name != "unknown":
                logging.debug(f"Detected version pattern: {pattern_name} for version: {version}")
                return pattern_name
            ...
            logging.error(f"Error in determine_version_pattern for version '{version}': {str(e)}")
            return "unknown"